class AmputationLevelArea extends React.Component {
  ...
  loadAmutationLevelArea() {
    const amputationLevelOptions = this.props.levels.map(level => {
      return (
        <option disabled={this.isSupportedAmputationLevel(level) ? "" : "disabled"} value={level.id} key={level.id} >
  ...
  render() {
    const classes = this.props.showAmputationLevelArea ? 'accordion-head active' : 'accordion-head';
    ...
    return (
      <div className={classes}>
        ...
        {this.props.showAmputationLevelArea ? this.loadAmutationLevelArea() : ""}
        <p>{this.state.areaSelected}{this.state.percentSelected}</p>
      </div>
    );
  }
};
